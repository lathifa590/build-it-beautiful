                ...
                <div className="mt-3 text-sm text-amber-900 bg-amber-100/50 p-3 rounded-lg border border-amber-200/50">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <button
                      type="button"
                      onClick={async () => {
                        if (confirm('Yakin ingin membatalkan semua sisa antrean dokumen ini?')) {
                          await supabase
                            .from('generation_queue')
                            .delete()
                            .eq('workspace_id', workspace.id)
                          ...
                          const { data: workspaces, error } = await supabase
                            .from('workspaces')
                            .select('id, is_autogenerated');
                          // Simple reload is easiest since we are modifying DB directly
                          window.location.reload();
                        }
                      }}
                      className="text-xs px-2 py-1 bg-red-100 hover:bg-red-200 text-red-700 rounded-md font-semibold transition-colors shadow-sm border border-red-200"
                    >
                      Batalkan Semua
                    </button>
                  </div>
