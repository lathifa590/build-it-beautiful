...
export const WorkspaceMeetingEditor: React.FC<WorkspaceMeetingEditorProps> = ({
  ...
  const handleAutoGeneratedFields = (autoGenerated: any) => {
    if (!autoGenerated || (!autoGenerated.auto_generated && !autoGenerated.identifikasi_murid)) return;
    setFormData((prev) => (prev ? applyAutoGeneratedFields(prev, autoGenerated) : prev));
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'materi' && formData) {
      // Materi lama diganti → bersihkan isian auto-fill agar AI mengisi ulang sesuai materi baru.
      if (formData.materi && value !== formData.materi && hasAutoFillContent(formData)) {
        toast({ description: 'Materi diganti — isian AI Auto-fill dibersihkan agar diisi ulang sesuai materi baru.' });
      }
      setFormData((prev) => (prev ? applyMateriChange(prev, value) : prev));
    } else {
