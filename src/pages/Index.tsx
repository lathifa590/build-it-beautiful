...
import { applyAutoGeneratedFields, applyMateriChange, hasAutoFillContent } from '@/lib/autofill';
...
const Index = () => {
  ...
  const handleInputChange = (
    ...
    if (name === 'fase') {
      ...
      // Materi lama diganti → isian auto-fill modul sebelumnya tidak relevan lagi.
      // Bersihkan agar AI mengisi ulang sesuai materi baru (bukan menempel ke modul awal).
      if (formData.materi && value !== formData.materi && hasAutoFillContent(formData)) {
        showNotificationMessage('Materi diganti — isian AI Auto-fill dibersihkan agar diisi ulang sesuai materi baru.');
      }
      setFormData((prev) => applyMateriChange(prev, value));
    } else {
  ...
  // Mapping konsisten dengan Workspace — lihat src/lib/autofill.ts
  const handleAutoGeneratedFields = (data: any) => {
    if (data?.auto_generated || data?.identifikasi_murid) {
      setFormData((prev) => applyAutoGeneratedFields(prev, data));
